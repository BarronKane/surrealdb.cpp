// The probe `scripts/compat-matrix.sh` builds under every supported toolchain
// configuration.
//
...
void probe_values() {
    ...
    // The linear bulk path.
    std::vector<sdb::owned_value> held;
    std::vector<sr_value_t> block;
    for (int i = 0; i < 4; ++i) {
        held.emplace_back(::sr_value_int(i));
        block.push_back(*held.back().get());
    }
    auto bulk = sdb::array_from(block.data(), static_cast<int>(block.size()));
...
int main() {
    std::printf("compat probe: c++%ld ranges:%d span:%d expected:%d\n",
                static_cast<long>(SURREALDB_CPLUSPLUS),
                SURREALDB_HAS_RANGES, SURREALDB_HAS_SPAN,
